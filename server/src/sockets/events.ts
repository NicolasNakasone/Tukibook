...
export const handleSocketEvents = (io: Server, socket: Socket): void => {
  ...
  socket.on(SocketEvents.COMMENT_POST, (post: Post) => {
    io.emit(SocketEvents.COMMENT_POST, post) // Emite el comentario que se creo a todos los clientes conectados
  })

  socket.on(SocketEvents.EDIT_COMMENT, (post: Post) => {
    io.emit(SocketEvents.EDIT_COMMENT, post) // Emite el comentario que se edito a todos los clientes conectados
  })

  socket.on(SocketEvents.DELETE_COMMENT, (post: Post) => {
    io.emit(SocketEvents.DELETE_COMMENT, post) // Emite el comentario que se borro a todos los clientes conectados
  })
}
